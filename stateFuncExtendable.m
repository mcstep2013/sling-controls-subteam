function xdot = stateFuncExtendable(x, u, pvstate)
% twolinkStateFcn
...
xdot(2)=x2d;
xdot(3)=x3d;
xdot(4)=2*x2d + x1 - (1-mu)*(x1+mu)/(d^3) - mu*(x1-1+mu)/(r^3) + u(1);
xdot(5)=-2*x1d + x2 - (1-mu)*x2/(d^3) - mu*x2/(r^3) + u(2);
xdot(6)=-(1-mu)*x3/(d^3) - mu*x3/r^3 + u(3);
