...
% x0Attacker=[0.9;0;pi/2;-1.8;0.9*(g/4);0]; %Attacker requires little input to come near the target
x0Attacker=[0.9;-0.2;3*pi/4;0;0.7*(g/4);0]; % A neat set of ICs where the attacker is headed towards the target, but the defender gets in its way and it needs to turn around
% x0Attacker=[0.9;-0.1;3*pi/4;0;0.7*(g/4);0]; %Attacker seems to make it over defender and hits the target?
...
msobj_a.ManipulatedVariables(2).Min = -1;
msobj_a.ManipulatedVariables(2).Max = 1;
...
for k = 1:(Duration/Ts)
    ...
    %Commenting these out to hopefully run faster
