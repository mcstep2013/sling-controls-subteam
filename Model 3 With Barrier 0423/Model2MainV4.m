...
%% A few pairs of initial conditions
...
%Attacker seems to make it over defender and hits the target?
% x0 = [-0.1; 0.15; 0; 0];
...
x0=[-pi/4;0.7*pi;0;0];
x0Attacker=[0.9;-0.1;3*pi/4;0;0.7*(g/4);0];
Duration = 1.5; % simulation duration (s)
...
Q_a=[30;30;0;0;0;0]; %state weight matrix
...
xd=[1;1;0;0]; %placeholder for defender state
...
msobj_a.ManipulatedVariables(1).Min = -0.3;
msobj_a.ManipulatedVariables(1).Max =  0.3;
msobj_a.ManipulatedVariables(2).Min = -2;
msobj_a.ManipulatedVariables(2).Max = 2;
...
%% Attacker
%This section is now defunct since attacker is now controlled by its own
%nmpc
...
% May need to pick a smarter initial guess (guess is not of state, but of x
% xd xdd y yd ydd)
x0_guess2 = [0; 0; 0; 45; 0; 0];
% Random EKF inputs, may need to tune a bit
initial_covariance2 = diag([1, 1, 1, 1, 1, 1]);
process_noise2 = diag([0.01; 0.01; 0.01; 0.01; 0.01; 0.01]);
measure_noise2 = diag([0.5; 1e-6; 0.2; 0.1]);
% set up constant acceleration EKF
filter2 = trackingEKF(State=x0_guess, ...
                     StateCovariance=initial_covariance, ...
                     StateTransitionFcn=@constacc, ...
                     ProcessNoise=process_noise, ...
                     MeasurementFcn= @(state) cameas(state, 'spherical'), ...
...
attPlot=scatter(xHistoryAttacker(1,1),xHistoryAttacker(1,2),30,'Color','r','Marker','o');
ghostDefenderPlot=scatter(x_est2(1,1),x_est2(4,1),30,'Color',[120, 133, 191],'Marker','o');
ghostAttackerPlot=scatter(x_est(1,1),x_est(4,1),30,'Color',[176, 108, 107],'Marker','o');
axis equal
legend('Defender','Defender Tip Bounds','Attacker','Estimated Defender','Estimated Attacker')
xlim([-0.2,1])
ylim([-0.2,1])
exportgraphics(gcf,'testAnimated.gif','Append',true);
for k=2:length(xDefender)
    ...
    ghostDefenderPlot.XData=x_est2(1,k);
    ghostDefenderPlot.YData=x_est2(4,k);
