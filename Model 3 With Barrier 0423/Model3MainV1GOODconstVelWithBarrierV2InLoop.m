...
Duration = 7.0; %1.0; % simulation duration (s)
...
Q_a=[30;30;0;1.5;1.5;0.1];
Sf_a=10*Q_a; %Terminal state weight matrix
R_a=[0;0]; %Control Weight matrix
barrierScaling=[2;3]; %outer and inner scaling constant
finalBarrierScaling=[10*barrierScaling(1);barrierScaling(2)];
...
pvcost_a=[xTarget;Sf_a;Q_a;R_a;p_a;xd;avoidRadius; killRadius; barrierScaling; finalBarrierScaling];
...
for k = 1:p_a+1
    msobj_a.Stages(k).CostFcn = "attackerCostFcnV2WBarrier";
    msobj_a.Stages(k).CostJacFcn = "attackerCostJacFcnV2WBarrier";
...
msobj_a.ManipulatedVariables(1).Min = -2;
msobj_a.ManipulatedVariables(1).Max =  2;
...
%% Setting up loop through initial conditions
x0 = [-pi/4; pi/2; 0; 0]; %initial conditions for defender do not change.
x0_guess  = [0.528; 0.679; 0; 0; 0; 0];
x0_guess2 = [0.01; -0.35; 0; 0]; %initial guesses not changing either I guess

x0Attacker=[NaN; NaN; pi/2; 0; 0.0; 0.0];

xPos0Range=[0,1.2];
xPos0delta=0.1;
yPos0Range=[1;-1];
yPos0delta=-0.1;
numXPos0=round(1+(xPos0Range(2)-xPos0Range(1))/xPos0delta);
numYPos0=1+(yPos0Range(2)-yPos0Range(1))/yPos0delta;

xPos0=repmat(xPos0Range(1):xPos0delta:xPos0Range(2),numYPos0,1);
yPos0=repmat([yPos0Range(1):yPos0delta:yPos0Range(2)]',1,numXPos0);

resultsMatrix=zeros(size(xPos0));

for initialIter1=numYPos0:-1:1
    for initialIter2=1:numXPos0

        x0Attacker(1)=xPos0(initialIter1,initialIter2);
        x0Attacker(2)=yPos0(initialIter1,initialIter2);
        ...
        endK=(Duration/Ts);
        for k = 1:endK
            fprintf('%i of %i, x loop %i of %i, y loop %i of %i \n',k,endK,initialIter2,numXPos0,initialIter1,numYPos0);
            
            %% Checking if its time to break out of simulation loop
            currentTime=(k-1)*Ts;
            xDefender=L1*cos(xHistory1(k,1))+L2*cos(xHistory1(k,1)+xHistory1(k,2));
            yDefender=L1*sin(xHistory1(k,1))+L2*sin(xHistory1(k,1)+xHistory1(k,2));
            xRel=xDefender-xHistoryAttacker(k,1);
            yRel=yDefender-xHistoryAttacker(k,2);
            rRel=(xRel^2 +yRel^2)^0.5;
            if ((xHistoryAttacker(k,1)^2+xHistoryAttacker(k,2)^2)^0.5 < targetRadius)%if attacker reaches target
                resultsMatrix(initialIter1,initialIter2)=currentTime;
                break;
            elseif rRel<killRadius %if defender reaches attacker
                resultsMatrix(initialIter1,initialIter2)=NaN;
                break;
            elseif k==endK %if attacker does not reach defender before we time out (ie, defender blocks attacker)
                resultsMatrix(initialIter1,initialIter2)=inf;
                break;
            end
            ...
            for iter=1:p+1
                ...
                elseif dist<minR %if the attacker is too close to reach
                    ...
                    th2A=real(acos((xe^2 + ye^2 -L1^2 -L2^2)/(2*L1*L2)));
                    %the real() is necesarry because sometimes small numerical errors cause th2A to be something like
                    %0+(1e-8)j, when it should just be 0
            ...
            for iter=1:p_a+1
                ...
                [xcart,xdcart,~,ycart,ydcart,~]=ThetaToCart(th1,th1dot,0,th2,th2dot,0,L1,L2);
                tempVar=[tempVar; [pvcost_a(1:21);xcart;ycart;xdcart;ydcart;pvcost_a(26:31)]];
