...
% syms x y xdot ydot
% syms tau1 tau2
syms xStateFull [4,1] real
syms tauIn [2,1] real
syms sysParams [9,1] real;
x=xStateFull(1);
y=xStateFull(2);
xdot=xStateFull(3);
ydot=xStateFull(4);
...
% using isAlways(xTestPrelim==x), we are not able to prove that what we're
% doing works. But that doesnt prove it doesnt work either so screw it
...
%% Computing Jacobians
%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
xstatefull=[x; y; xdot; ydot];
dfdx=jacobian(xdotout,xstatefull);
dfdtau=jacobian(xdotout,tauIn);

%% Creating a function for them
JacobianFunc=matlabFunction(dfdx, dfdtau,'Vars', {xStateFull, tauIn, sysParams}, ...
          'File', 'TwoLinkArmJacobiansCartesian');
