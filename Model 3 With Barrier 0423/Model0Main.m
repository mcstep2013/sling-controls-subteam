...
msobj.Model.StateFcn = "TwoLinkArmEOMCartesian";
msobj.Model.StateJacFcn = "TwoLinkArmJacobiansCartesian";
...
for k = 1:p+1
    ...
    % msobj.Stages(k).IneqConFcn=something
end

msobj.ManipulatedVariables(1).Min = -8.5;
msobj.ManipulatedVariables(1).Max =  8.5;
msobj.ManipulatedVariables(2).Min = -8.5;
msobj.ManipulatedVariables(2).Max =  8.5;

maxX=0.5*sqrt(2*(L1+L2)^2);
minX=-maxX;
maxY=maxX;
minY=-maxY;

msobj.States(1).Max=maxX;
msobj.States(1).Min=minX;
msobj.States(2).Max=maxY;
msobj.States(2).Min=minY;
...
for k = 1:(Duration/Ts)
    ...
    ODEFUN = @(t,xk) TwoLinkArmEOMCartesian(xk,uk,pvstate);
